...
const App = () => {
  ...
  const handleCreate = async (blog) => {
    ...
    const returnedBlog = await blogService.create(blog).catch((err) => {
      console.log(err);
    });
    setNotificationMessage(
      `a new blog ${returnedBlog.title} by ${blog.author} added!`
    );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to remove this blog?")) {
      ...
      setBlogs(blogsCopy);
      setNotificationMessage(
        `a new blog ${blogTitle} by ${blogAuthor} removed!`
      );
