...
beforeEach(async () => {
  ...
  let res = await api.post('/api/login').send({username: 'testuser', password: 'test'} )
  let login = res.body
  await Blog.deleteMany({})
  const blogObjects = helper.initialBlogs
  const promiseArray = blogObjects.map(blog => api.post('/api/blogs').set('authorization', `bearer ${login.token}` ).send(blog))
